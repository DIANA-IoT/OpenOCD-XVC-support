...
static int read_frame(int sock_id, unsigned char *ptr, unsigned int size)
{
	...
	while (i > 0) {
		...
		if (state == 0) {
			...
			return ERROR_FAIL; /* report error to the caller */
		}
		if (state < 0) {
...
static int xvc_flush(void)
{
	...
	unsigned int shift_len = strlen(shift);
...
static int xvc_getinfo(void)
{
	...
	unsigned int len = strlen(getinfo);
	...
	ssize_t read = read_socket(xvc_fd, info_recv_buf, sizeof(info_recv_buf));
	if (read < 0) {
...
static int xvc_set_tck(void)
{
	...
	ssize_t read = read_socket(xvc_fd, &tck_recv_buf, sizeof(tck_recv_buf));
	if (read < 0) {
		LOG_ERROR("%s: read", __func__);
		return ERROR_FAIL;
	}
